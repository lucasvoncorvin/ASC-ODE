This report will discuss several topics while dealing with the analyses of specific problems that can be modelled using ODE's and solving them numerically with different methods.
...
m y''(t)=-k y(t)
$ 
, where $m$ is the mass and k the spring constant. This is a ODE of second order, so first we need to rewrite as a problem of first order:
...
Before looking into the numerical schemes, we should analyse what solution we should expect from this problem. If we look at the system, it becomes clear, that it is a Hamiltonian system and therefore we expect a solution that is conservative w.r.t time. Also if we study the system as a matrix system, we get imaginary eigenvalues with real part = 0. This indicates, that solution of the system must be closed circles in a phase plot.

In the following, we will investigate 4 different methods:
Explicit Euler, Explicit Midpoint Rule (Runge Kutta 2), Implicit Euler and Crank Nicholson Method.
Each of them were presented in the lecture.
For the implicit methods, we need to use the Newtons method in order to solve for the next time step, since it involves solving an implicit equation.
This method also has been presented in class and is basically calculating the 0 of a given function by using Taylor expansion.
...
For the explicit midpoint scheme and the crank nicholson scheme, both of them are symplectic integrators, so we expect almost perfectly closed circles even for smaller time steps. The simulation goes from $t_0=0$ until $t_{end}=4 \pi$
...
Crank-Nicholson 100 time steps
<p float="left">
